...
{book_project_name} has two types of caches.  One type of cache sits in front of the database to decrease load on the DB
...
This cache is a local cache.  Local caches do not use replication even if you are in the cluster with more {book_project_name} servers.
...
NOTE:  More advanced configuration of these caches can be found in the  link:{book_appserver_caching_link}[Infinispan] section of the _{book_appserver_caching_name}_.
