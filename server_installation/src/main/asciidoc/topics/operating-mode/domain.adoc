...
This capability is built into the {book_appserver_name} Application Server which {book_project_name} derives from.
...
many {book_project_name} server instances will be started on that machine.  When the host controller boots up, it starts
as many {book_project_name} server instances as it was configured to do.  These server instances pull their configuration
...
where the domain profile and server group for the {book_project_name} server are defined.


.domain.xml
image:{book_images}/domain-file.png[]
...
{book_project_name} server instance.  Any value that contains `${...}` is a value that can be overriden on the command line
...
The definition of the server group for {book_project_name} resides in the `server-groups` XML block.  It specifies the domain profile
...
{book_project_name} comes with two host controller configuration files that reside in the _.../domain/configuration/_ directory:
...
one {book_project_name} server instance.  _host-slave.xml_ is configured to talk to the domain controller and boot up
one {book_project_name} server instance.
...
image:{book_images}/host-files.png[]
...
Each {book_project_name} server instance defined in your host files creates a working directory under _.../domain/servers/{SERVER NAME}_.
...
The structure of these per server directories ends up looking like any other {book_appserver_name} booted server.

.Working Directories
image:{book_images}/domain-server-dir.png[]
...
.Domain Boot Script
image:{book_images}/domain-boot-files.png[]
...
* 2 {book_project_name} server instances
...
{book_project_name} authentication server instance.  The second will be a slave host controller that only starts
...
NOTE: The add-user.sh does not add user to {book_project_name} server but to the underlying JBoss Enterprise Application Platform. The credentials used and generated in the above script are only for example purpose. Please use the ones generated on your system.
