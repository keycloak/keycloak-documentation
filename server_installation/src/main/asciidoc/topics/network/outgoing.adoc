...
The {book_project_name} server often needs to make non-browser HTTP requests to the applications and services it secures.
...
When {book_project_name} invokes on remote HTTPS endpoints, it has to validate the remote server's certificate in order to ensure it is connecting to a trusted server.
...
certificates must be put in a truststore.  This truststore is managed by the {book_project_name} server.
