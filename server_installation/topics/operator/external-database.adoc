...
The {project_operator} will create a Service that connects to an external database, such as `keycloak-postgresql`, that connects {project_name} to the Database that contains the IP address or Service name that is specified by the POSTGRES_EXTERNAL_ADDRESS.
