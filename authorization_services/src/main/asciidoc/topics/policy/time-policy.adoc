...
.Add Time Policy
image:{book_images}/policy/create-time.png[alt="Add Time Policy"]
...
Access is only granted if all conditions are satisfied. {book_project_name} will perform an _AND_ based on the outcome of each condition.