...
{book_project_name} comes with a fully functional Admin REST API with all features provided by the Admin Console.
...
{book_adminguide_link}[{book_adminguide_name}].

A token can be obtained by enabling authenticating to your application with {book_project_name}; see the
{book_adapterguide_link}[{book_adapterguide_name}]. You can also use direct access grant to obtain an access token.

For complete documentation see {book_apidocs_link}[{book_apidocs_name}].
...
Complete Javadoc for the admin client is available at {book_apidocs_link}[{book_apidocs_name}].
endif::[]