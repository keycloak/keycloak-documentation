...
For these types of situations, {book_project_name} allows you to augment your external store by storing extra information
in {book_project_name}'s database.  This is called federated user storage and is encapsulated within the
...
and required actions.  If your external store's datamodel  cannot support the full {book_project_name} feature
set, then this service can fill in the gaps.

{book_project_name} comes with a helper class `org.keycloak.storage.adapter.AbstractUserAdapterFederatedStorage`
