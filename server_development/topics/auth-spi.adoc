...
===== Implementing an Authenticator: Project Prerequisites

Before implementing a custom Authenticator, we need to import the Keycloak dependency containing the Authenticator
interface which is defined within the artifact `keycloak-server-spi-private` using the Keycloak release version.
Also, the dependencies `keycloak-server-spi-private` and `keycloak-server-spi-private` are required to access other
parts of the Keycloak server and inbound HTTP request parts such as Cookies.

The POM snippet should look similar to the following:

[source,xml]
----
    <properties>
        <keycloak.version>4.8.1.Final</keycloak.version> <!--1-->
        <resteasy.version>3.6.1.Final</resteasy.version> <!--2-->
    </properties>
...
----
<1> Define a version property to simplify upgrading
<2> The RestEasy dependency is required to access HTTP request parts, such as Cookies.

After loading the above dependencies into the project, we may start implementing an Authenticator.

==== Implementing an Authenticator

When implementing the Authenticator interface, we first need to implement the `requiresUser()` method.
In this example, the `requiresUser()` method must return true as we need to validate the secret question
associated with the user. A provider like kerberos would return false from this method as it can resolve a user
from the negotiate header. This example, however, is validating a specific credential of a specific user.
...
In our case we are storing this information, hashed, within a UserCredentialValueModel within the UserModel
(just like passwords are stored).  Here's how we do this very simple check:

[source,java]
----
    /**
     * The type string separates this Authenticator from other Authenticators.
     */
    public static final String SECRET_QUESTION_TYPE = "secret_question";

    @Override
    public boolean configuredFor(final KeycloakSession session,
                                 final RealmModel realm,
                                 final UserModel user) {

        return session.userCredentialManager().isConfiguredFor(realm, user, SECRET_QUESTION_TYPE);
    }
----            

The isConfiguredFor() call queries the user to see if it supports that credential type.

The next method to implement on the Authenticator is setRequiredActions().
If configuredFor() returns false and our example authenticator is required within the flow, this method will be called.
...
----
    /**
     * Called if this Authenticator is <em>not</em> configured for the provided user (i.e. if the
     * {@link #configuredFor(KeycloakSession, RealmModel, UserModel)} method returned <code>false</code>).
     * This method defines required actions which must be performed by the user before this {@link Authenticator}
     * can be used.
     *
     */
    @Override
    public void setRequiredActions(final KeycloakSession session,
                                   final RealmModel realm,
                                   final UserModel user) {

        user.addRequiredAction("SECRET_QUESTION_CONFIGURATION");
    }
----            

Now we are getting into the meat of the Authenticator implementation.
The next method to implement is authenticate(), which is the initial method the flow invokes when the execution is
first visited.
What we want is that if a user has answered the secret question already on their browser's machine, then the user
doesn't have to answer the question again, making that machine "trusted".  The authenticate() method isn't responsible
for processing the secret question form. Its sole purpose is to render the page or to continue the flow.
...
----
    ...
    public void authenticate(final AuthenticationFlowContext context) {
        ...
        if (hasValidCookie(context, "secQConfig")) {

            context.success();
            return;
        }

        final Response challenge = context.form().createForm("secret_question.ftl");
        context.challenge(challenge);
    }
----            

The hasValidCookie() method checks to see if there is already a cookie set on the browser which indicates that the
secret question has already been answered. If that returns true, we just mark this execution's status as SUCCESS using
the AuthenticationFlowContext.success() method and returning from the authentication() method. One such example method
is provided below, where the important bit is to illustrate how to access HTTP request data from within the
AuthenticationFlowContext. Note that the AuthenticationFlowContext uses RestEasy to access the underlying HTTP data,
which implies a need for the `resteasy-jaxrs` dependency as illustrated above.

[source,java]
----
    /**
     * Checks a named Cookie in the underlying HTTP request, and verifies that its value can be
     * parsed into a LocalDate in the future.
     *
     * @param context    The active AuthenticationFlowContext.
     * @param cookieName A non-empty name of a Cookie.
     * @return True if a Cookie with the supplied name exists within the underlying HTTP request, and its
     * value is a LocalDate object in the future.
     */
    public static boolean hasValidCookie(final AuthenticationFlowContext context, final String cookieName) {

        final boolean canValidateCookie = cookieName != null
                && !cookieName.trim().isEmpty()
                && context != null
                && context.getHttpRequest() != null
                && context.getHttpRequest().getHttpHeaders() != null
                && context.getHttpRequest().getHttpHeaders().getCookies() != null
                && context.getHttpRequest().getHttpHeaders().getCookies().get(cookieName) != null;

        boolean toReturn = false;

        if (canValidateCookie) {

            final Map<String, Cookie> cookieMap = context.getHttpRequest().getHttpHeaders().getCookies();
            final Cookie cookie = cookieMap.get(cookieName);

            if (cookie != null) {

                // Valid Cookies have a String value parse-able to a future LocalDate.
                try {
                    final LocalDate lastValidityDate = (LocalDate) DateTimeFormatter.ISO_DATE.parse(cookie.getValue());
                    toReturn = lastValidityDate.isAfter(LocalDate.now());
                } catch (Exception e) {
                    // Ignore this.
                }
            }
...
If the hasValidCookie() method returns false, we must return a response that renders the secret question HTML form.
AuthenticationFlowContext has a form() method that initializes a Freemarker page builder with appropriate base
information needed to build the form. This page builder is called `org.keycloak.login.LoginFormsProvider`.
the LoginFormsProvider.createForm() method loads a Freemarker template file from your login theme.
Additionally you can call the LoginFormsProvider.setAttribute() method if you want to pass additional information to
the Freemarker template. We'll go over this later.
...
This sets the status of the execution as CHALLENGE and if the execution is Required, this JAX-RS Response object will
be sent to the browser.

So, the HTML page asking for the answer to a secret question is displayed to the user and the user enters in the
...
----
    ...
    protected void setCookie(final AuthenticationFlowContext context) {

        final AuthenticatorConfigModel config = context.getAuthenticatorConfig();

        int maxCookieAge = (int) Duration.ofDays(30).toMillis();
        if (config != null) {
