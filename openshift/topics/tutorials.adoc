...
+
[source,bash,subs="attributes+,macros* The steps described in the xref:Preparing-SSO-Authentication-for-OpenShift-Deployment[Preparing {project_name} Authentication for OpenShift Deployment] section have been performed already.
...
[role="_additional-resources"]
.Additional resources
* link:{project_doc_base_url}/server_administration_guide/#export_import[Export and import]

==== (Optional) Creating additional realm and users to be exported
...
. link:{project_doc_base_url}//server_administration_guide/#create-realm[Create a new realm]
. link:{project_doc_base_url}//server_administration_guide/#create-new-user[Create new users]

Upon their creation, the database can be exported.

[role="_additional-resources"]
.Additional resources
* link:{project_doc_base_url}//server_administration_guide/#export_import[{project_name} {project_version} Export and import]
...
[NOTE]
====
For importing into an {project_name} server not running on OpenShift, see the link:{project_doc_base_url}/server_administration_guide/#export_import[Export and Import section] of the RH SSO Server Administration Guide.
====
+
When the {project_name} server is running as a {project_name} {project_version} container on OpenShift, use the link:{project_doc_base_url}//server_administration_guide/#admin_console_export_import[Admin Console Export/Import] function to import the resources from previously exported JSON file into the {project_name} server's database
...
[role="additional-resources"]
.Additional resources
* link:{project_doc_base_url}//server_administration_guide/#clients[Managing Clients]
...
+
The OpenShift login page now has the option to use either *htpasswd_auth* or *rh-sso*. The former is still available because it is present in the */etc/origin/master/master-config.yaml*.
...
[[binary-builds]]
=== Creating OpenShift Application from  Maven binaries and securing it using {project_name}
...
----
. Configure the link:https://access.redhat.com/maven-repository[Red Hat JBoss Middleware Maven repository].
