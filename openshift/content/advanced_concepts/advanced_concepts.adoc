These cover further additional configuration topics, like setup of keystores and truststore for the RH-SSO server, creating an administrator account, overview of available RH-SSO client registration methods, or guidance on how to configure clustering.
...
[NOTE]
====
For simplicity, in the following command all the provided passwords (the one for the HTTPS keystore, the one for the JGroups keystore, the one for the RH-SSO server truststore, and finally the one for RH-SSO administrator user account) have been chosen to be identical. Although for production environments Red Hat recommends that you use your local password policy to ensure the passwords are different and strong enough. Yet, *the passwords provided when provisioning the template needs to match the passwords provided when creating the keystores*.
