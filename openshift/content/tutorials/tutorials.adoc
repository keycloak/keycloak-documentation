...
+
It is *required* to specify xref:../advanced_concepts/advanced_concepts.adoc#sso-administrator-setup[*_SSO_ADMIN_USERNAME_* and *_SSO_ADMIN_PASSWORD_* credentials] here. If not specified, they are auto generated and displayed as an OpenShift instructional message when the template is instantiated.
+
[cols="2*", options="header"]
|===
|Variable
...
=== Example Workflow: Creating OpenShift Application that Authenticates Using Red Hat Single Sing-On from Existing Maven Binaries
