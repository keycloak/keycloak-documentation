...
To use more than the link:#internal-token-to-internal-token-exchange[Internal Token to Internal Token Exchange] flow, you should also enable the `admin_fine_grained_authz` feature. For details, see  link:{installguide_profile_link}[{installguide_profile_name}]. 
