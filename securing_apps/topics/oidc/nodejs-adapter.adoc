...
being used. You may also provide the Path to a json file (relative to the project directory).

[source,javascript]
----
    let keycloak = new Keycloak({ store: memoryStore }, './config/keycloak.json');
----

As alternative, it's also possible to provide a configuration
