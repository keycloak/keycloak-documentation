...
For SPAs, there's a the possibility to use a _silent refresh_ of the access token when using the implicit flow.
This is achieved with the help of a hidden iframe in the application.
Therefore, the init parameter `silentRefreshRedirectUri` needs to be provided on {project_name} initialization.
...
The page at the silent refresh uri is loaded in the iframe after successfully obtaining new tokens from the {project_name} server.
...
If you have the {project_name} Javascript adapter available as a separate UMD, you can also use this handy code for the page at the silent refresh uri:
...
Please keep in mind, that this page at the specified location must be provided by the application and is _not_ part of the JavaScript adapter!

Now, the access token can be refreshed using the `updateToken` method, like with the standard Authorization Code flow.
...
* silentRefreshRedirectUri - Set the redirect uri for silent refresh of access token for implicit flow.
