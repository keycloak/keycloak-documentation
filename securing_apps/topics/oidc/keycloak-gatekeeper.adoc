...
In order to customize the above Header names created out of claims, one should pass in the claims with a | separator, with the latter part to be the new header name
for eg :

[source,yaml]
----
add-claims:
- preferred_username|Impersonate-User
----

would add addition headers to authenticated request with custome header names like :
...
Note that the claim headers are processed as a map so the latter ones will replace the existing previous keys
...
will result in header like below to be sent.

[source,bash]
----
X-Auth-Impersonate-User: username
----
