...
Keycloak Gatekeeper can be run as a sidecar container. Example deployment configuration for a container, deployment and service named my-app:
  ...
  template:
    ...
    spec:
      ...
      - image: keycloak/keycloak-gatekeeper:7.0.0
        ...
        args:
        - --discovery-url=https://<KEYCLOAK_HOST>/auth/realms/<REALM> # Note that when KEYCLOAK_HOST runs in Kubernetes too the host's Ingress should have whitelisted internal IPs. I.e., 10.0.0.0/8.
        - --client-id=<CLIENT_ID>
        - --client-secret=<CLIENT_SECRET>
