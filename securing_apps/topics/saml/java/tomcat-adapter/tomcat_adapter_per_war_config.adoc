...
In case a file location is required for `keycloak-saml.xml` (in cases of shared files and/or runtime configuration). It is possible to user a file location parameter  name "keycloak.config.file", as in the example bellow:

[source,xml]
----
<Context path="/your-context-path">
    <Valve className="org.keycloak.adapters.saml.tomcat.SamlAuthenticatorValve"/>
    <Parameter name="keycloak.config.file" value="/shared/site/conf/keycloak-saml.xml" />
</Context>
----
