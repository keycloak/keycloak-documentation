...
{book_project_name} adapter subsystem. You then have two options to secure your WARs.
endif::[]
ifeval::[{book_product}==true]
...
Alternatively, you don't have to modify your WAR at all and you can secure it via the {book_project_name} adapter subsystem configuration in `standalone.xml`.
...
----
$ cd $WILDFLY_HOME
$ unzip keycloak-wildfly-adapter-dist-{book_project_version}.zip
----
...
----
$ cd $WILDFLY_HOME
$ unzip keycloak-wf8-adapter-dist-{book_project_version}.zip
----
...
----
$ cd $EAP_HOME
$ unzip keycloak-eap7-adapter-dist-{book_project_version}.zip
----
...
----
$ cd $EAP_HOME
$ unzip keycloak-eap6-adapter-dist-{book_project_version}.zip
----
...
----
$ cd $JBOSS_HOME
$ unzip keycloak-as7-adapter-dist-{book_project_version}.zip
----
endif::[]
...
----
$ cd $EAP_HOME
$ unzip rh-sso-{book_project_version}-eap7-adapter.zip
----
...
----
$ cd $EAP_HOME
$ unzip rh-sso-{book_project_version}-eap6-adapter.zip
----
endif::[]

This ZIP archive contains JBoss Modules specific to the {book_project_name} adapter. It also contains JBoss CLI scripts
...
You do not have to modify your WAR to secure it with {book_project_name}. Instead you can externally secure it via the {book_project_name} Adapter Subsystem.
...
This metadata is instead defined within server configuration (i.e. `standalone.xml`) in the {book_project_name} subsystem definition.
...
To make it easier for you, you can go to the {book_project_name} Administration Console and go to the Client/Installation tab of the application this WAR is aligned with.
