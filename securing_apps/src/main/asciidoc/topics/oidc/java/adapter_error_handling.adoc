...
{book_project_name} has some error handling facilities for servlet based client adapters.
When an error is encountered in authentication, {book_project_name} will call `HttpServletResponse.sendError()`.
You can set up an error-page within your `web.xml` file to handle the error however you want.
{book_project_name} can throw 400, 401, 403, and 500 errors.
...
{book_project_name} also sets a `HttpServletRequest` attribute that you can retrieve.
