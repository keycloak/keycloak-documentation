...
Multi Tenancy, in our context, means that a single target application (WAR) can be secured with multiple {book_project_name} realms. The realms can be located
one the same {book_project_name} instance or on different instances.
...
{book_project_name} makes it possible to have a custom config resolver so you can choose what adapter config is used for each request.
