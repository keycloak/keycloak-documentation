...
Each adapter is a separate download on the {book_project_name} download site.
...
After adding the modules, you must then enable the {book_project_name} SAML Subsystem within your app server's server configuration: `domain.xml` or `standalone.xml`.
