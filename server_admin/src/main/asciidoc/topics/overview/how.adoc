...
{book_project_name} is a separate server that you manage on your network.  Applications are configured to point to and
be secured by this server.  {book_project_name} uses open protocol standards like link:http://openid.net/connect[Open ID Connect]
or link:http://saml.xml.org/saml-specifications[SAML 2.0] to secure
your applications.  Browser applications redirect a user's browser from the application to the {book_project_name} authentication
