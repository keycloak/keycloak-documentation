...
image:{book_images}/user-required-action.png[]
...
.Default Required Actions
image:{book_images}/default-required-actions.png[]
...
of the website.  {book_project_name} has this functionality implemented as a required action, but it requires some configuration.  For one, you
...
_terms.ftl_ file in the _base_ login theme.  See the link:{book_developerguide_link}[{book_developerguide_name}] for more information on extending and
creating themes.
