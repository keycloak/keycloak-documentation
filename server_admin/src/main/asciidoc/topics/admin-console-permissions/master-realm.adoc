...
The `master` realm in {book_project_name} is a special realm and treated differently than other realms.
Users in the {book_project_name} `master` realm can be granted permission to manage zero or more realms that are deployed on the {book_project_name} server.
When a realm is created, {book_project_name} automatically creates various roles that grant fine-grain permissions to access that new realm.
...
Each realm in {book_project_name} is represented by a client in the `master` realm.
