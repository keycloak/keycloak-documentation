...
{book_project_name} supports <<_saml,SAML 2.0>> for registered applications.
...
.Add Client
image:{book_images}/add-client-saml.png[]
...
URL you want the {book_project_name} server to send SAML requests and responses to.  Usually applications have only one URL for processing SAML requests.
...
.Client Settings
image:{book_images}/client-settings-saml.png[]



Client ID::
  This value must match the issuer value sent with AuthNRequests.
  {book_project_name} will pull the issuer from the Authn SAML request and match it to a client by this value.

Name::
  This is the display name for the client whenever it is displayed in a {book_project_name} UI screen.  You can localize
  the value of this field by setting up a replacement string value i.e. $\{myapp}.  See the link:{book_developerguide_link}[{book_developerguide_name}]
...
Consent Required::
  ...
  If you've ever done a social login to Google, you'll often see a similar page.  {book_project_name} provides the same functionality.
...
Sign Documents::
  When turned on, {book_project_name} will sign the document using the realm's private key. 

Optimize REDIRECT signing key lookup::
  When turned on, the SAML protocol messages will include {book_project_name}
  ...
  IDP and SP are provided by {book_project_name} server and adapter. This
...
SAML Signature Key Name::
  ...
  element. This by default contains {book_project_name} key ID. However various vendors might
...
Client Signature Required::
  ...
  {book_project_name} will validate this signature using the client public key or cert set up in the `SAML Keys` tab.

Force POST Binding::
  By default, {book_project_name} will respond using the initial SAML binding of the original request.
  By turning on this switch, you will force {book_project_name} to always respond using the SAML POST Binding even if the original request was the Redirect binding.
...
Front Channel Logout::
  ...
  If this switch is false, then {book_project_name} will invoke a background SAML request to logout the application.
...
Root URL::
  If {book_project_name} uses any configured relative URLs, this value is prepended to them.
...
Valid Redirect URIs::
  ...
  endpoints are not registered and {book_project_name} is pull the Assertion Consumer URL from the request.

Base URL::
  If {book_project_name} needs to link to the client, this URL would be used.
