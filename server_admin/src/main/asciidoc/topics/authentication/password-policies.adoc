...
as insecure a password, as they want.  Simple settings are fine for development or learning {book_project_name},
but unacceptable in production environments.  {book_project_name} has a rich set of password policies you can enable
...
.Password Policy
image:{book_images}/password-policy.png[]
...
image:{book_images}/failed-password-policy.png[]
...
HashAlgorithm::
  ...
  The only built-in and default algorithm available is PBKDF2. See the link:{book_developerguide_link}[{book_developerguide_name}]
