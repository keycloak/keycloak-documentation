...
image:{book_images}/microsoft-add-identity-provider.png[]
...
page is the `Redirect URI`.  You'll have to provide that to Microsoft when you register {book_project_name} as a client there, so
...
You'll have to copy the `Redirect URI` from the {book_project_name} `Add Identity Provider` page and add it to the
`Redirect URIs` field on the Microsoft application page.  Be sure to click the `Add Url` button and `Save` your changes.

Finally, you will need to obtain the Application ID and secret from this page so you can enter them back on the {book_project_name} `Add identity provider` page.
