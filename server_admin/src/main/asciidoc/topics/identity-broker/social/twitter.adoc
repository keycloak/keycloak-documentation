...
image:{book_images}/twitter-add-identity-provider.png[]
...
page is the `Redirect URI`.  You'll have to provide that to Twitter when you register {book_project_name} as a client there, so
...
`Callback URL` you must copy the `Redirect URI` from the {book_project_name} `Add Identity Provider` page.
...
Finally, you will need to obtain the API Key and secret from this page and copy them back into the `Client ID` and `Client Secret` fields on the {book_project_name} `Add identity provider` page.
