...
{book_project_name} can broker identity providers based on the SAML v2.0 protocol.
...
image:{book_images}/saml-add-identity-provider.png[]
...
|Signed SAML documents sent via POST binding contain identification of signing key in `KeyName`
 element. This by default contains {book_project_name} key ID. However various external SAML IDPs might
...
If you are connecting to a {book_project_name} external IDP, you can import the IDP setttings from the url `<root>/auth/realms/\{realm-name}/protocol/saml/descriptor`.
