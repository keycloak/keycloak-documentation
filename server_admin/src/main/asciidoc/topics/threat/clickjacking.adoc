...
By default, every response by {book_project_name} sets some specific browser headers that can prevent this from happening.
...
By default, {book_project_name} only sets up a _same-origin_ policy for iframes.
