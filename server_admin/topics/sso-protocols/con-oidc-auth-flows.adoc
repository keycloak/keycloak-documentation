...
After logout, user will be automatically redirected to the specified `post_logout_redirect_uri` as long as it is provided as a parameter.
Note that it is needed to include either `client_id` or `id_token_hint` parameter in case that `post_logout_redirect_uri` is included. Also the `post_logout_redirect_uri` parameter
needs to match with one of the `Valid Post Logout Redirect URIs` specified in the client configuration.
