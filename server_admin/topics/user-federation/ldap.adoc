...
Filter Registrations::
  Switch it on if you want to sync registrations only for users with a username that matches Regular Expression Filter. If the username does not match the regular expression, {project_name} will try lower
  priority providers and eventually use local storage to register the new user.
