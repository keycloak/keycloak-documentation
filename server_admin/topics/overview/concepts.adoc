...
clients::
  ...
  want to use {project_name} to secure themselves and provide a single sign-on solution (i.e. acting as OAuth2 Resource Server).  Clients can also be entities that just want to request
  identity information or an access token so that they can securely invoke other services on the network that are secured by {project_name} (i.e. the application in the browser on behalf of the user).
client adapters::
