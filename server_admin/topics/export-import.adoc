...
You can migrate the whole {project_name} database from one environment to another or migrate it to another database. The export/import triggers at server boot time, and its parameters pass through Java properties. 
