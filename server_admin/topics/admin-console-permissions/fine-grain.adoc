...
* Allowing regular expressions as valid redirect URIs for a specific OIDC-client.
...
==== Regular expressions as valid redirect URIs

To allow regular expressions as valid redirect URIs on a specific client the admin can use fine grain admin permissions. This only works with clients using the `openid-connect` protocol. The setup is similar to <<_managing_one_specific_client, Managing One Specific Client>>. 

.Client Permissions Tab
image:{project_images}/fine-grain-client-regex-permissions-tab-on.png[]

With `Permissions Enabled` on, clicking on `allow-regex-redirect-uri`  will take you to the permission that handles the `allow-regex-redirect-uri` permission in the `realm-management` client's `Authorization` tab.

.Client Allow-Regex-Redirect-URI Permission
image:{project_images}/fine-grain-client-regex-permission.png[]

Like in the <<_managing_one_specific_client, Managing One Specific Client>> section apply the `sales-app-admin` policy with the client `sales-application` to the permission. `Resource` and `Scopes` are being configured automatically.

Now you can navigate to the settings of the client `sales-application` to set regular expressions as valid redirect URIs.

.Regular Expression as valid redirect URI
image:{project_images}/fine-grain-client-regex-settings.png[]
...
map-roles-client-scope::
    ...
    This is a shortcut, easy-of-use feature to avoid having to define policies
    for each and every role defined by the client.
allow-regex-redirect-uri::
    Policies that decide if a OIDC-client can configure regular expressions as valid redirect uris.
