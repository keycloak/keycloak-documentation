...
For non-web cases or when the ticket is not available during login, {project_name} also supports login with Kerberos username/password.
...
. {project_name} returns to the application.
...
For the MIT Kerberos example above, we already exported keytab to `/tmp/http.keytab` . If your KDC and {project_name} are running on the same host,
...
Once you install https://www.docker.com/[docker], you can run a docker image with FreeIPA server installed.
FreeIPA provides an integrated security solution with MIT Kerberos and 389 LDAP servers among other things. The image provides also {project_name}
...
will allow the users from realm A to access resources (services) of realm B. This means that realm B trusts realm A.

.Kerberos cross-realm trust
image:images/kerberos-trust-basic.png[]


The {project_name} server has support for cross-realm trust. There are a few things which need to be done to achieve this:
...
this principal has the same keys on both Kerberos realms. This is usually achieved when the principals have the same password, key version number
and there are the same ciphers used in both realms. It is recommended to consult the Kerberos server documentation for more details.
...
that LDAP can find users from both realms A and B. We want to improve this limitation in future versions, so you can
...
* Enable TRACE logging for category `org.keycloak` in the logging section of `standalone/configuration/standalone.xml` to receive more info `standalone/log/server.log`
