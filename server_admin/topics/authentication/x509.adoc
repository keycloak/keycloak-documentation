...
. Select *Duplicate* from the "ellipses button" to make a copy of the built-in "Browser" flow.
...
. Click *Authentication* in the menu.
ifeval::[{project_community}==true]
. Select *Duplicate* from the "ellipses button" to make a copy of the built-in "Direct grant" flow.
. Enter a name for the copy.
. Click *Duplicate*.
. Click the created flow.
. Click on the *🗑️* (trash can icon) of the "Username Validation" and click *Delete*.
. Click on the *🗑️* (trash can icon) of the "Password" and click *Delete*.
. Click *Add step*.
