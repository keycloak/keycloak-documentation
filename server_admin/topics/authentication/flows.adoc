...
. Enter the required maximum number of sessions a user can have in this realm. For example if configured to 2, then each user can have at
maximum 2 SSO sessions in this realm. If a value of 0 is used, this check is disabled.
. Enter the required maximum number of sessions a user can have for the client. For example if configured to 2, then each user can have at
maximum 2 SSO sessions in this realm for each client. So when user is trying to authenticate to client `foo`, but he already has 2 SSO sessions where he is authenticated
to client `foo`, then the authentication won't be permitted (or one of the existing sessions will be killed based on the behaviour configured). If a value of 0 is used, this check is disabled.
If both session limits and client session limits are enabled, it makes sense to have client session limits to be always lower than session limits as limit per client can never be bigger than limit of all SSO sessions of this user.
...
Authenticator should be added at the point when the user is already known during authentication (usually at the end of the authentication flow) and should be typically REQUIRED. Note that it is not possible to have
ALTERNATIVE and REQUIRED executions at the same level and hence for example for the default browser flow, it may be needed to wrap existing flow as a REQUIRED level-1 subflow and then
