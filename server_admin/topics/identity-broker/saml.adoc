...
|A numeric index that identifies the attribute set to request to the remote IDP. Attribute sets and their indexes should be manually added to the client metadata document and shared with the IDP. If empty, the IDP decides which attribute set to return.
|===
