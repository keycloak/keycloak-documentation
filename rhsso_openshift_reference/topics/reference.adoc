...
|*_redhat-sso-7/sso70-openshift_* or *_redhat-sso-7/sso71-openshift_*
...
|Password of the administrator account for the `master` realm of the SSO
...
|Username of the administrator account for the `master` realm of the SSO
server. *Required.* If no value is specified, it is auto generated and
displayed as an OpenShift Instructional message when the template is
instantiated.
|*_admin_*
...
|*_SSO_REALM_*
|Name of the realm to be created in the SSO server if this environment variable
is provided.
|*_demo_*

|*_SSO_SERVICE_PASSWORD_*
|The password for the SSO service user.
|*_mgmt-password_*

|*_SSO_SERVICE_USERNAME_*
|The username used to access the SSO service. This is used by clients to create
the application client(s) within the specified SSO realm. This user is created
...
Available link:https://docs.openshift.com/container-platform/3.4/dev_guide/templates.html#overview[application templates]
...
deployed SSO-enabled application will align with the intended use case as much
as possible. The list of configuration variables, available for each category
of application templates for SSO-enabled applications, is described below.

==== Template variables for all SSO images

.Configuration Variables Available For All SSO Images
...
|The name of the service account to use for the deployment. The service account
should be configured to allow usage of the secret(s) specified by
*_HTTPS_SECRET_* and *_JGROUPS_ENCRYPT_SECRET_*.
...
==== Template variables specific to *sso70-mysql*, *sso71-mysql*, *sso70-mysql-persistent*, and *sso71-mysql-persistent*

.Configuration Variables Specific To SSO-enabled MySQL Applications With Ephemeral Or Persistent Storage
[cols="2*", options="header"]
|===
|Variable
|Description
...
==== Template variables specific to *sso70-postgresql*, *sso71-postgresql*, *sso70-postgresql-persistent*, and *sso71-postgresql-persistent*

.Configuration Variables Specific To SSO-enabled PostgreSQL Applications With Ephemeral Or Persistent Storage
[cols="2*", options="header"]
|===
|Variable
|Description
...
==== Template variables specific to *sso70-mysql-persistent*, *sso71-mysql-persistent*, *sso70-postgresql-persistent*, and *sso71-postgresql-persistent*

.Configuration Variables Specific To SSO-enabled MySQL / PostgreSQL Applications With Persistent Storage
[cols="2*", options="header"]
|===
|Variable
|Description
...
==== Template variables for general *eap64* and *eap70* s2i images
...
==== Template variables specific to *eap64-sso-s2i* and *eap70-sso-s2i* for automatic client registration

.Configuration Variables For EAP 6.4 and EAP 7 SSO-enabled Applications Built Via S2I
[cols="2*", options="header"]
|===
|Variable
|Description
...
|*_SSO_USERNAME_*
|The username used to access the SSO service. This is used to create the
application client(s) within the specified SSO realm. This should match the
*_SSO_SERVICE_USERNAME_* specified through one of the *sso70-* or *sso71-* templates.

|*_SSO_PASSWORD_*
|The password for the SSO service user.

|*_SSO_PUBLIC_KEY_*
|SSO public key. Public key is recommended to be passed into the template to
...
|If true SSL communication between EAP and the SSO Server is insecure
...
==== Template variables specific to *eap64-sso-s2i* and *eap70-sso-s2i* for automatic client registration with SAML clients

.Configuration Variables For EAP 6.4 and EAP 7 SSO-enabled Applications Built Via S2I Using SAML Protocol
[cols="2*", options="header"]
|===
|Variable
|Description
